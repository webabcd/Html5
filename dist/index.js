(function () {
  ...
  */
   ...
   *
   */
  var a = ['webabcd']; // 自己构造 blob 对象时，第一个参数是一个数组，其成员可以是字符串或 arrayBuffer 或 blob 等
  // 一般是不需要自己构造 blob 对象的，通常都是从 XMLHttpRequest 或 fetch() 或 <input type='file' /> 获取 blob 对象，然后再做相关处理的

  var b = new Blob(a, {
    type: 'text/plain'
  });
  console.log(b.type, b.size); // blob 就是一个文件对象，需要通过 FileReader 处理

  var reader = new FileReader(); // 读取 blob 完成了
  ...
  var c = window.URL.createObjectURL(b); // 结果类似 blob:http://localhost:63342/241ccac3-7f5c-42d1-a2b0-e539c99235ee

  document.write("<a href='".concat(c, "'>click me</a>")); // 浏览器跳转到这个 url 后会显示 webabcd
